...
fig.suptitle('Dashboard RLM | Spotify - ms_played ~ shuffle + skipped', fontsize=16, fontweight='bold')

# Arriba izquierda: Residuos vs Predichos
axes[0, 0].scatter(predicciones_completas, residuales, color='blue', alpha=0.3, s=10)
axes[0, 0].axhline(0, color='red', linestyle='--', linewidth=2)
axes[0, 0].set_xlabel('ms_played Predicho')
axes[0, 0].set_ylabel('Residuales')
axes[0, 0].set_title('Residuos vs Predichos')
axes[0, 0].grid(True, alpha=0.3)

# Arriba derecha: QQ-plot
sm.qqplot(residuales, line='s', ax=axes[0, 1])
axes[0, 1].set_title('QQ-plot de Residuales')

# Abajo izquierda: Histograma de residuales
axes[1, 0].hist(residuales, bins=50, color='blue', alpha=0.7, edgecolor='black')
axes[1, 0].set_xlabel('Residuales')
axes[1, 0].set_ylabel('Frecuencia')
axes[1, 0].set_title('Distribución de Residuales')
axes[1, 0].grid(True, alpha=0.3, axis='y')

# Abajo derecha: Real vs Predicho
axes[1, 1].scatter(df['ms_played'], predicciones_completas, color='green', alpha=0.3, s=10)
min_val = min(df['ms_played'].min(), predicciones_completas.min())
max_val = max(df['ms_played'].max(), predicciones_completas.max())
axes[1, 1].plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=2, label='y=x')
axes[1, 1].set_xlabel('ms_played Real')
axes[1, 1].set_ylabel('ms_played Predicho')
